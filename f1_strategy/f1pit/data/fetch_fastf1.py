...
def _load_circuit_info() -> pd.DataFrame:
    """Load CircuitInfo.csv from data directory."""
    path = PATHS.data_circuit_info
    if not path.exists():
...
def fetch_all_seasons(
    ...
    # ── Additional thesis-required files ──

    # Pitstops.csv: same as PitstopsWithTeams but without Team column
    if all_pitstops:
        pitstops_no_team = pd.concat(all_pitstops, ignore_index=True)
        pitstops_no_team = pitstops_no_team.drop(columns=["Team"], errors="ignore")
        pitstops_no_team = pitstops_no_team.loc[:, ~pitstops_no_team.columns.duplicated()]
        pitstops_no_team.to_csv(output_dir / "Pitstops.csv", index=False)
        LOGGER.info("Saved Pitstops.csv: %s rows", len(pitstops_no_team))

    # SafetyCars2024.csv: safety car data for 2024 only (test set)
    if all_safety_cars:
        sc_all = pd.concat(all_safety_cars, ignore_index=True)
        sc_2024 = sc_all[sc_all["Year"] == 2024].copy()
        if not sc_2024.empty:
            sc_2024.to_csv(output_dir / "SafetyCars2024.csv", index=False)
            LOGGER.info("Saved SafetyCars2024.csv: %s rows", len(sc_2024))
        else:
            LOGGER.warning("No 2024 safety car data for SafetyCars2024.csv")

    # Copy CircuitInfo.csv into processed/ for convenience
    ci_src = PATHS.data_circuit_info
    ci_dst = output_dir / "CircuitInfo.csv"
    if ci_src.exists() and not ci_dst.exists():
        import shutil
        shutil.copy2(ci_src, ci_dst)
        LOGGER.info("Copied CircuitInfo.csv to %s", ci_dst)
